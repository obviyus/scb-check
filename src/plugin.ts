...
function booleanReturn(statement: ESTree.Statement): boolean | undefined {
  ...
  create(context) {
    return {
      CatchClause(node) {
        ...
        if (value === null ||
          (value.type === "Literal" && (value.value === null || value.value === true || value.value === false || value.value === 0 || value.value === "")) ||
...
const noBooleanReturnBranches = defineRule({
  meta: {
    type: "suggestion",
...
const noIdenticalTernaryBranches = defineRule({
  meta: {
    type: "suggestion",
...
const noNestedOnlyIf = defineRule({
  meta: {
    type: "suggestion",
...
const noDoubleAssertion = defineRule({
  meta: {
    type: "suggestion",
  ...
  create(context) {
    function check(node: ESTree.TSAsExpression | ESTree.TSTypeAssertion) {
      ...
      if ((inner.type === "TSAsExpression" || inner.type === "TSTypeAssertion") &&
        (inner.typeAnnotation.type === "TSAnyKeyword" || inner.typeAnnotation.type === "TSUnknownKeyword")) {
...
const noDuplicateBlocks = defineRule({
  meta: {
    type: "suggestion",
