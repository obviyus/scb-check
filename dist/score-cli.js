...
async function main() {
  ...
  const packageSchema = z2.object({ name: z2.string(), version: z2.string() });
  ...
  if (exitCode !== 0 && exitCode !== 1)
    throw new Error(`Oxlint did not complete (exit ${exitCode}): ${stdout || stderr}`);
  const report = {
  ...
  if (values.json)
    ...
    const percent = (value) => value === null ? "not available" : `${(value * 100).toFixed(2)}%`;
...
main().catch((error) => {
  console.error(`Cannot compute scores: ${error.message}`);
  process.exitCode = 2;
});
